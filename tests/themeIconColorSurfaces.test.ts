...
/**
 * The Appearance toggle that forces theme colours, on every surface that
 * draws a space icon.
 *
 ...
 *
 * The check is always the same: with the toggle on there is NO inline colour,
 * which is what lets the theme's `--icon-color` apply. Computed colour is no
...
const SPACES: SpaceDefinition[] = [
  { id: "a", name: "Garden", icon: "leaf", color: COLOUR, members: [] },
];
...
function rowIcon(useThemeIconColor: boolean): HTMLElement {
  ...
  return appendSpaceIcon(
    row,
    { key: { kind: "space", id: "a" }, label: "Garden", icon: "leaf", color: COLOUR, active: false },
    useThemeIconColor
  );
}

describe("forcing theme colours on every surface that draws a space icon", () => {
  it("paints the space's colour on the strip with the toggle off", async () => {
    expect((await stripIcon(false)).style.color).toBe("rgb(78, 205, 196)");
  });

  it("writes no colour on the strip with the toggle on", async () => {
    expect((await stripIcon(true)).style.color).toBe("");
  });

  it("paints the space's colour in the header with the toggle off", async () => {
    expect((await headerIcon(false)).style.color).toBe("rgb(78, 205, 196)");
  });

  it("writes no colour in the header with the toggle on", async () => {
    // The surface the neutral swatch fix missed the first time.
    expect((await headerIcon(true)).style.color).toBe("");
  });

  it("paints the space's colour on a list row with the toggle off", async () => {
    expect(rowIcon(false).style.color).toBe("rgb(78, 205, 196)");
  });

  it("writes no colour on a list row with the toggle on", async () => {
  ...
  it("gives the colour back when the toggle goes off again", async () => {
    // The whole promise of the setting. Nothing was written to the space, so
    // a strip built after the toggle flips paints the stored colour again.
    ...
    expect(defs.get().spaces[0].color).toBe(COLOUR);
  });
});
